...
func CreateOrUpdateVMCluster(ctx context.Context, cr *v1beta1.VMCluster, rclient client.Client, c *conf.BaseOperatorConf) (string, error) {
	...
	if cr.Spec.VMStorage != nil {
		...
		err = performRollingUpdateOnSts(ctx, rclient, vmStorageSts.Name, cr.Namespace, cr.VMStorageSelectorLabels())
		if err != nil {
			reason = v1beta1.StorageRollingUpdateFailed
			return status, err
		}

		//create vmstorage service
	...
	if cr.Spec.VMSelect != nil {
		...
		err = performRollingUpdateOnSts(ctx, rclient, vmSelectsts.Name, cr.Namespace, cr.VMSelectSelectorLabels())
		if err != nil {
...
func CreateOrUpdateVMStorageService(ctx context.Context, cr *v1beta1.VMCluster, rclient client.Client, c *conf.BaseOperatorConf) (*corev1.Service, error) {
	...
	l.Info("vmstorage svc reconciled")
	return newService, nil
...
func genVMSelectSpec(cr *v1beta1.VMCluster, c *conf.BaseOperatorConf) (*appsv1.StatefulSet, error) {
	...
	if _, ok := cr.Spec.VMSelect.Resources.Limits[corev1.ResourceCPU]; !ok {
		cr.Spec.VMSelect.Resources.Limits[corev1.ResourceCPU] = resource.MustParse(c.VMSingleDefault.Resource.Limit.Cpu)
	}

	if _, ok := cr.Spec.VMSelect.Resources.Requests[corev1.ResourceMemory]; !ok {
		cr.Spec.VMSelect.Resources.Requests[corev1.ResourceMemory] = resource.MustParse(c.VMSingleDefault.Resource.Request.Mem)
	}
	if _, ok := cr.Spec.VMSelect.Resources.Requests[corev1.ResourceCPU]; !ok {
		cr.Spec.VMSelect.Resources.Requests[corev1.ResourceCPU] = resource.MustParse(c.VMSingleDefault.Resource.Request.Cpu)
	}
	...
	stsSpec := &appsv1.StatefulSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:            cr.Spec.VMSelect.GetName(cr.Name),
		...
		Spec: appsv1.StatefulSetSpec{
			...
			Template:    *podSpec,
			ServiceName: cr.Spec.VMSelect.GetName(cr.Name),
		},
	}
	if cr.Spec.VMSelect.CacheMountPath != "" {
		...
		if storageSpec == nil {
			stsSpec.Spec.Template.Spec.Volumes = append(stsSpec.Spec.Template.Spec.Volumes, corev1.Volume{
				Name: cr.Spec.VMSelect.GetCacheMountName(),
			...
			if pvcTemplate.Name == "" {
				pvcTemplate.Name = cr.Spec.VMSelect.GetCacheMountName()
			}
...
func makePodSpecForVMSelect(cr *v1beta1.VMCluster, c *conf.BaseOperatorConf) (*corev1.PodTemplateSpec, error) {
	...
	if cr.Spec.VMStorage != nil && cr.Spec.VMStorage.ReplicaCount != nil {
		...
		for i := int32(0); i < vmstorageCount; i++ {
			storageArg += cr.Spec.VMStorage.BuildPodNameWithService(cr.Spec.VMStorage.GetName(cr.Name), i, cr.Namespace, cr.Spec.VMStorage.VMSelectPort)
		}
	...
	for i := int32(0); i < vmselectCount; i++ {
		selectArg += cr.Spec.VMSelect.BuildPodNameWithService(cr.Spec.VMSelect.GetName(cr.Name), i, cr.Namespace, cr.Spec.VMSelect.Port)
	}
...
func genVMInsertSpec(cr *v1beta1.VMCluster, c *conf.BaseOperatorConf) (*appsv1.Deployment, error) {
	...
	if _, ok := cr.Spec.VMInsert.Resources.Limits[corev1.ResourceCPU]; !ok {
		cr.Spec.VMInsert.Resources.Limits[corev1.ResourceCPU] = resource.MustParse(c.VMSingleDefault.Resource.Limit.Cpu)
	}

	if _, ok := cr.Spec.VMInsert.Resources.Requests[corev1.ResourceMemory]; !ok {
		cr.Spec.VMInsert.Resources.Requests[corev1.ResourceMemory] = resource.MustParse(c.VMSingleDefault.Resource.Request.Mem)
	}
	if _, ok := cr.Spec.VMInsert.Resources.Requests[corev1.ResourceCPU]; !ok {
		cr.Spec.VMInsert.Resources.Requests[corev1.ResourceCPU] = resource.MustParse(c.VMSingleDefault.Resource.Request.Cpu)
	}
	...
	stsSpec := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:            cr.Spec.VMInsert.GetName(cr.Name),
			Namespace:       cr.Namespace,
...
func makePodSpecForVMInsert(cr *v1beta1.VMCluster, c *conf.BaseOperatorConf) (*corev1.PodTemplateSpec, error) {
	...
	if cr.Spec.VMStorage != nil && cr.Spec.VMStorage.ReplicaCount != nil {
		...
		for i := int32(0); i < storageCount; i++ {
			storageArg += cr.Spec.VMStorage.BuildPodNameWithService(cr.Spec.VMStorage.GetName(cr.Name), i, cr.Namespace, cr.Spec.VMStorage.VMInsertPort)
		}
		storageArg = strings.TrimSuffix(storageArg, ",")
...
func GenVMStorageSpec(cr *v1beta1.VMCluster, c *conf.BaseOperatorConf) (*appsv1.StatefulSet, error) {
	...
	if !cpuResourceIsSet {
		cr.Spec.VMStorage.Resources.Requests[corev1.ResourceCPU] = resource.MustParse(c.VMSingleDefault.Resource.Request.Cpu)
		cr.Spec.VMStorage.Resources.Limits[corev1.ResourceCPU] = resource.MustParse(c.VMSingleDefault.Resource.Limit.Cpu)

	}
	if !memResourceIsSet {
		cr.Spec.VMStorage.Resources.Requests[corev1.ResourceMemory] = resource.MustParse(c.VMSingleDefault.Resource.Request.Mem)
	...
	stsSpec := &appsv1.StatefulSet{
		ObjectMeta: metav1.ObjectMeta{
			Name:            cr.Spec.VMStorage.GetName(cr.Name),
			Namespace:       cr.Namespace,
		...
		Spec: appsv1.StatefulSetSpec{
			...
			Template:    *podSpec,
			ServiceName: cr.Spec.VMStorage.GetName(cr.Name),
		},
	}
	storageSpec := cr.Spec.VMStorage.Storage
	if storageSpec == nil {
		stsSpec.Spec.Template.Spec.Volumes = append(stsSpec.Spec.Template.Spec.Volumes, corev1.Volume{
			Name: cr.Spec.VMStorage.GetStorageName(),
			VolumeSource: corev1.VolumeSource{
				EmptyDir: &corev1.EmptyDirVolumeSource{},
			},
		})
		...
		pvcTemplate := MakeVolumeClaimTemplate(storageSpec.VolumeClaimTemplate)
		if pvcTemplate.Name == "" {
			pvcTemplate.Name = cr.Spec.VMStorage.GetStorageName()
		}
		if storageSpec.VolumeClaimTemplate.Spec.AccessModes == nil {
			pvcTemplate.Spec.AccessModes = []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce}
		} else {
...
func makePodSpecForVMStorage(cr *v1beta1.VMCluster) (*corev1.PodTemplateSpec, error) {
	args := []string{
		...
		fmt.Sprintf("-storageDataPath=%s", cr.Spec.VMStorage.StorageDataPath),
...
func performRollingUpdateOnSts(ctx context.Context, rclient client.Client, stsName string, ns string, podLabels map[string]string) error {
	...
	if sts.Status.UpdateRevision != sts.Status.CurrentRevision {
		log.Info("sts update is neeeded", "sts", sts.Name, "current", sts.Status.CurrentRevision, "desired", sts.Status.UpdateRevision)
		stsVersion = sts.Status.UpdateRevision
	} else {
		stsVersion = sts.Status.CurrentRevision
	}
	l := log.WithValues("controller", "sts.rollingupdate", "desired version", stsVersion)
	...
	for _, pod := range podList.Items {
		if pod.Labels[podRevisionLabel] != stsVersion {
			l.Info("pod version doesnt match", "pod", pod.Name, "version", pod.Labels[podRevisionLabel])
			updatedNeeded = true
		}
	}
	if !updatedNeeded {
	...
	//we need list of pods in following order:
	// if pod is not ready
	...
	l.Info("updated pods with desired version:", "len", len(updatedPods))

	for _, pod := range updatedPods {
		l.Info("checking ready status for updated to desired version pod", "pod", pod.Name)
		err := waitForPodReady(ctx, rclient, ns, pod.Name)
		if err != nil {
...
func waitForPodReady(ctx context.Context, rclient client.Client, ns, podName string) error {
	time.Sleep(time.Second * 10)
	return wait.Poll(time.Second*4, time.Second*60, func() (done bool, err error) {
		pod := &corev1.Pod{}
		...
		if podIsReady(*pod) {
			log.Info("pod update finished with revesion", "pod", pod.Name, "revision", pod.Labels[podRevisionLabel])
			return true, nil
		}
		return false, nil
	})
